...
fn bench_add_same_size(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly Add - same size");
...
fn bench_add_different_sizes(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly Add - different sizes");
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
...
fn bench_add_assign_same_size(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly AddAssign - same size");
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
...
fn bench_add_assign_different_sizes(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly AddAssign - different sizes");
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
...
fn bench_sum_small_polys(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly Sum - small polynomials");
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
    ...
    for &n in num_polys.iter() {
        ...
        let polys: Vec<ZZPoly> = coeffs_data.iter()
            .map(|coeffs| ZZPoly::from(coeffs.clone()))
            .collect();
        
        group.bench_function(BenchmarkId::new("owned", n), |b| {
            b.iter(|| {
                let polys_clone = polys.clone();
                black_box(polys_clone.into_iter().sum::<ZZPoly>())
            })
        });
        
        group.bench_function(BenchmarkId::new("ref", n), |b| {
            b.iter(|| black_box(polys.iter().sum::<ZZPoly>()))
        });
    }
    group.finish();
}
...
fn bench_sum_medium_polys(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly Sum - medium polynomials");
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
...
fn bench_sum_varying_sizes(c: &mut Criterion) {
    let mut group = c.benchmark_group("ZZPoly Sum - varying polynomial sizes");
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
