...
mod tests {
    ...
    fn test_from_raw() {
        ...
        let poly = ZZPoly::from_raw(coeffs);
    ...
    #[test]
    fn test_normalize() {
        let mut poly = ZZPoly::from(vec![Integer::from(1), Integer::from(2), Integer::from(0), Integer::from(0)]);
    ...
    fn test_normalize_all_zeros() {
        let mut poly = ZZPoly::from(vec![Integer::from(0), Integer::from(0), Integer::from(0)]);
    ...
    fn test_with_capacity() {
        let poly = ZZPoly::with_capacity(10);
        assert_eq!(poly.length(), 0);
        assert!(poly.capacity() >= 10);
        assert!(poly.is_zero());
    }

    #[test]
    fn test_zero() {
        let poly = ZZPoly::zero();
        assert_eq!(poly.length(), 0);
        assert!(poly.is_zero());
    }
    ...
    #[test]
    fn test_is_zero() {
        let zero_poly = ZZPoly::zero();
        assert!(zero_poly.is_zero());

        let non_zero_poly = ZZPoly::from(vec![1]);
        assert!(!non_zero_poly.is_zero());
    }

    #[test]
    fn test_is_one() {
        let one_poly = ZZPoly::from(vec![1]);
        assert!(one_poly.is_one());

        let not_one_poly = ZZPoly::from(vec![2]);
        assert!(!not_one_poly.is_one());

        let zero_poly = ZZPoly::zero();
        assert!(!zero_poly.is_one());

        let polynomial_poly = ZZPoly::from(vec![1, 2]);
    ...
    fn test_is_gen() {
        ...
        let gen_poly = ZZPoly::from(vec![0, 1]);
        assert!(gen_poly.is_gen());

        let not_gen_poly = ZZPoly::from(vec![1, 1]);
        ...
        let higher_degree_poly = ZZPoly::from(vec![0, 1, 1]);
